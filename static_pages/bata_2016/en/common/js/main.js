...
function documentready(){
	...
	if(/android.+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od|ad)|iris|kindle|lge|maemo|meego.+mobile|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent||navigator.vendor||window.opera)
		...
		xmlrequest1 = getXMLHTTPRequestObject('https://www.bayareafastrak.org/vector/homepage/HomePage.do?fbpz=skip&exclGen=true&ajax=chat','updateNavigationItems', '');
...
function changeInputTypes(){	
	...
	function updateNavigationItems(){
		...
		$("li[id $='_nav_panel']").each(function(index, element) {
            if(panel == this.id){
				$(this).show();	
			}else{
				$(this).hide();
	...
	function accountLogin(action){
		...
		document.getElementById("password").value = document.getElementById("pWord").value;
		action.document.forms["accountlogin"].submit();
	}
	function lookup(form){
                        var inviol= document.getElementById("violation").value;
                        var plate = document.getElementById("license").value;
                        var state = document.getElementById("states").value;
                        if(inviol.startsWith('T') || inviol.startsWith('t')){
                                    document.getElementById("violationNumber").value = inviol;
                                    document.getElementById("licensePlate").value = plate;
                                    document.getElementById("licenseState").value = state;
                                    document.getElementById("btnLookupViolation").value = "1";
                                    form.document.forms["frmViolationInquiry"].action="/vector/violations/bataViolationInquiry.do";
                                    form.document.forms["frmViolationInquiry"].submit();
                        }else{
                                    document.getElementById("invoiceNumber").value = inviol;
                                    document.getElementById("licensePlate").value = plate;
                                    document.getElementById("licenseState").value = state;
                                    document.getElementById("btnSearch").value = "1";
                                    form.document.forms["frmViolationInquiry"].action="/vector/invoices/invoiceInquiry.do";
                                    form.document.forms["frmViolationInquiry"].submit();
                        }
            }

	function tagreg(){
		document.location="/vector/account/transponders/registerTollTag.do?exclGen=true&btnRegisterNewAccount=1&exclGen=true&tollTagNumber="+document.getElementById("tollTag").value+"&validationCode="+document.getElementById("validationTag").value;
	}
	function fillChars(val){
		var filler="00000000000";
		if(!!document.getElementById('tollTag') && val != null && val != ""){
			document.getElementById('tollTag').value=filler.substring(0,11-val.length)+val;
		}
	}